...
* Suppose $X$ is a random variable that counts the number of tosses needed for a head to come up the first time. Its PMF is 
    ...
    * $(1 - p)^{k-1}p$ is the probability of the sequence consisting of $k-1$ successive trials followed by a head. This is a valid PMF because
    ...
    $$\sum_{k=1}^{\infty} p_X(k) = \sum_{k=1}^{\infty} (1 - p)^{k-1}p = p \sum_{k=1}^{\infty} (1 - p)^{k-1} = p \times \frac{1}{1 - (1-p)} = 1$$
...
* Suppose $X$ is a random variable that takes on values $X \in \{0, 1, 2, \ldots, \}$ and that $P(X = k) = p(k)$ is,
    ...
    \end{eqnarray}
    $$
* Not enough time for the proof here (find it online), but $E(X) = var(X) = \lambda$
...
* What is the probability the president will make ten or more threats?
    ...
    $$
    \begin{eqnarray}
    P(X \geq 10) & = & e^{-\lambda} \sum_{k=10}^{\infty} \frac{5^{k}}{k!} \\
    & = & 1 - P(X< 10 )
    \end{eqnarray}
    $$
...
* All outcomes in the sample space defined with a numerical value $x$ for $X$ also have a numberical value $y = g(x)$ for $Y$
